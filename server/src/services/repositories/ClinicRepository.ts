...
export default class ClinicRepository implements ClinicInterface {
  save = async (clinic: Clinic) => await getRepository(TypeormClinic).save(clinic)

  getById = async (id: number) => await getRepository(TypeormClinic).findOne({ id })

  getByAddress = async (address: string) => await getRepository(TypeormClinic).find()
